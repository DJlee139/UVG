...
import com.google.android.apps.uvg.data.LumenDataPoint;
import com.google.android.apps.uvg.weka.Globals;
 ...
 */
public class TrackRecordingService extends Service implements SensorEventListener{
  ...
  //Light Sensor executor Service
  private ExecutorService lightSensorExecutorService;
  ...
  //JERRID:
  //Light sensor file
  private static final Object LIGHT_SENSOR_LOCK = new Object();
  private static final String LIGHT_SENSOR_FILE ="_classification_recording.txt";
  File root = android.os.Environment.getExternalStorageDirectory(); 
  File dir = new File (root.getAbsolutePath() + "/lightsensor");
  //-----------------
  
  public static final long MILLISECOND=1;
  public static final long NANOSECOND=MILLISECOND*1000000;
  public static final long MINUTE = 60*ONE_SECOND;
  //Sensing window interval: setup for 18MS default
  public static final long WINDOW_INTERVAL=320*MILLISECOND;

  //Toggle for sensor
  private static boolean START_SENSING=false;
  
  //Delay between first sensing
  public static final long SENSING_DELAY=2*ONE_SECOND;
  public static final long FIRST_EXECUTATION_DELAY=2*ONE_SECOND;
    
    long startTime=0;
    float[] mGravity,mGeomagnetic;
    float mLightReading=0, currentLightReading=0,uviReading=-1;
    double pitchReading=0;

  private android.hardware.SensorManager mSensorManager;
  private android.hardware.Sensor mAccelerometer,mMagnetometer,mLightSensor;


  // Timer to periodically invoke checkLocationListener
  private Timer lightTimer = new Timer();
  private TimerTask lightSensorListener = new TimerTask() {
      @Override
    public void run() {
      if (isRecording() && !isPaused()) {
        TrackRecordingService.START_SENSING=true;
        //Reset the sensing start time since window was reset
        startTime=System.currentTimeMillis();

        Log.e("Start Sensing: ","Sensor Start Sensing "+startTime);
      }
    }
  };
  //---------------------------------
  ...
  private MyTracksProviderUtils myTracksProviderUtils;//Wrapper to interface w/ Tracks Application DB instance Handler
  ...
  private void registerLightSensor(){
    mSensorManager.registerListener(this, mLightSensor, android.hardware.SensorManager.SENSOR_DELAY_FASTEST);        
    mSensorManager.registerListener(this, mMagnetometer, android.hardware.SensorManager.SENSOR_DELAY_FASTEST);        
    mSensorManager.registerListener(this,mAccelerometer, android.hardware.SensorManager.SENSOR_DELAY_FASTEST);
    
    //schedule timer to start the periodic sensing for light information
    if(lightTimer != null){
      lightTimer.cancel();     
      lightTimer = new Timer();
      lightTimer.schedule(lightSensorListener, TrackRecordingService.FIRST_EXECUTATION_DELAY, TrackRecordingService.SENSING_DELAY);
    }
  }
  
  //Unregister the sensors when encssary
  ...
  public void onCreate() {
    ...
    mLightSensor = mSensorManager.getDefaultSensor(android.hardware.Sensor.TYPE_LIGHT);
    mMagnetometer = mSensorManager.getDefaultSensor(android.hardware.Sensor.TYPE_MAGNETIC_FIELD);

    lightSensorExecutorService = Executors.newSingleThreadExecutor();
    registerLightSensor();
          ...
          while (true) {
              try 
              {
                  ...
                  if (blockSize == Globals.LIGHT_BLOCK_CAPACITY) 
                  {
                      ...
                      //double prediction = classifier.classifyInstance(featureInstance);
                      ...
                      try {
                          sunClassificationFile = new FileWriter(dir.getAbsolutePath()+"/"+recordingTrackId+LIGHT_SENSOR_FILE, true);                 
      
                          //Write the XYZ accelerometer points to file
                          synchronized (LIGHT_SENSOR_LOCK) {
                              try {                                  
  ...
  /*
  private void onSensorChangedAsync( SensorEvent event){
      ...
      if(isRecording() && !isPaused())
      {
          ...
          final float timestamp= event.timestamp;
          final long currTime= System.currentTimeMillis();
      
          //Kill sensing because past the sensing window interval
          //Note: The timer will restart it again for the next interval
          if(Math.abs(currTime-startTime) >= TrackRecordingService.WINDOW_INTERVAL){
            TrackRecordingService.START_SENSING=false;
            Log.e("Stopped Sensing: ","Sensor Stopped Sensing " +currTime);
          }
      
          currentLightReading = Math.round(x);//current light sensor reading
          ...
          if (mGravity != null && mGeomagnetic != null) 
          {
               ...
               if (success) 
               {
                   ...
                   //     public void run() {
                            dir.mkdirs();
                            FileWriter lightsFile=null;
                        
                            try {
                                lightsFile = new FileWriter(dir.getAbsolutePath()+"/"+recordingTrackId+LIGHT_SENSOR_FILE, true);                 
            
                                //Write the XYZ accelerometer points to file
                                synchronized (LIGHT_SENSOR_LOCK) {
                                    try {
                                        
                                        lightsFile.append(timestamp+"\t"+currentLightReading+"\t"+pitchReading+"\t"+uviReading+"\n");           
                                    } catch (IOException ex){
                                    }
                                    finally{
                                        lightsFile.flush();
  ...
  */
  ...
  public void onSensorChanged( SensorEvent event) {  
      ...
      if (event.sensor.getType() == android.hardware.Sensor.TYPE_MAGNETIC_FIELD){
          ...
          if (mGravity != null && mGeomagnetic != null && TrackRecordingService.START_SENSING)
          {
              float R[] = new float[9];
              float I[] = new float[9];
              boolean success = android.hardware.SensorManager.getRotationMatrix(R, I, mGravity, mGeomagnetic);
              if (success) 
              {
                  ...
                  /*
                     lightSensorExecutorService.submit(new Runnable(){
                          @Override
                          public void run() {
                              onSensorChangedAsync(event);
                          }
                  });
                  */
              }
